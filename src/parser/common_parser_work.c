...
void	space_skip(char *str, size_t *i)
{
	while (str[*i] == ' ')
		(*i)++;
}
...
int	add_new_scene_arg(t_vlist **scene_arg, char *str, int *flag)
{
	...
	if (!ft_strncmp(str, "A ", 2))
		return (ambient_parsing(scene_arg, str, flag));
	if (!ft_strncmp(str, "C ", 2))
		return (camera_parsing(scene_arg, str, flag));
	if (!ft_strncmp(str, "L ", 2))
		return (light_parsing(scene_arg, str, flag));
	if (!ft_strncmp(str, "pl ", 3))
		return (plane_parsing(scene_arg, str, flag));
	if (!ft_strncmp(str, "sp ", 3))
		return (sphere_parsing(scene_arg, str, flag));
	if (!ft_strncmp(str, "cy ", 3))
		return (cylinder_parsing(scene_arg, str, flag));
	*flag = 0;
	return (0);
} //stop there
...
int	common_parser_work(char *argv, t_vlist **scene_arg)
{
	...
	while (str && flag)
	{
		if (add_new_scene_arg(scene_arg, str, &flag))
			return (free_vlist_and_std_error(scene_arg)); //need to erite
		free(str);
		str = get_next_line(fd);
	}
	if (!flag)
